...
def run_RNN(X, test, labels, path_to_glove, REPORT):
    ...
    for n_ in range(n_folds):
        ...
        model_filepath_weights = '../models/re_' + path_to_glove.split('/')[2].split('.')[0] + '_' + str(n_) + '.h5'
        model_filepath_json = '../models/re_' + path_to_glove.split('/')[2].split('.')[0] + '_' + str(n_) + '.json'
        ...
        if REPORT == 'yes':
            ...
            write_list(y_pred, cwd + '../models/re_RNN_predictions_' + model_name + '.txt', iterate=True, encoding=encoding)
...
if '__main__' == __name__:
    ...
    parser.add_argument('--train', type=str, default=None, help="""Data.""")
    ...
    path_to_glove = '../glove/vectors_triage_data_' + DATA + '_train.txt'
