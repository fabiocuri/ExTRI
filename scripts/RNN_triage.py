...
def run_RNN(X, test, labels, path_to_glove, REPORT):
    ...
    for n_ in range(n_folds):
        ...
        model_filepath_weights = '../models/triage_' + path_to_glove.split('/')[2].split('.')[0] + '_' + str(n_) + '.h5'
        model_filepath_json = '../models/triage_' + path_to_glove.split('/')[2].split('.')[0] + '_' + str(n_) + '.json'
        ...
        if REPORT == 'yes':
            ...
            write_list(y_pred, '../models/triage_RNN_predictions.txt', iterate=True, encoding=encoding)
...
if '__main__' == __name__:
    ...
    train = read_as_list('../glove/' + args.train + '.txt', encoding='latin-1')
    if REPORT == 'yes':
        test = read_as_list('../' + args.test + '.txt', encoding='latin-1')
    if REPORT == 'no':
        test = []
    labels = read_as_list('../triage_labels_train.txt', encoding='latin-1')
    path_to_glove = '../glove/vectors_' + args.train + '.txt'

    run_RNN(train, test, labels, path_to_glove, REPORT)
